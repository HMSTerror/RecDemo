...
\begin{itemize}
  ...
  % GATE-2 UPGRADE: if FOLLOWUP-09 confirms the inversion at user
  % level, promote the preflight proposal to its own contribution
  % bullet with the user-level correlation as second evidence.
...
\pending{FOLLOWUP-09 per-user analysis: within-dataset correlation of
per-user text utility with per-user first-generation delta (thousands
of users); include if it lands---upgrades the $n{=}4$ cross-dataset
evidence to user-level.}
...
($p{=}1/24$); the user-level analysis that would harden it is
\pending{pending (FOLLOWUP-09)}. By construction a closed gate can only
match the host---on high-utility datasets the method's value is safety,
not gains, and the gains column of the validation rests on the
open-gate datasets. The mechanism evidence comes from the
