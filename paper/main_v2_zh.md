...
**限制(如实)**:跨数据集逆序仅有四个点;exchangeability 假设下的 `1/24` 只是描述性组合计算。用户级读出不复现该逆序。冻结门验证包含一个关门 miss（ML1M）与一个 barely-open ATG（`φ=0.117`）miss。ML1M 超过测得的宿主噪声地板;ATG 为 provenance-limited 三次运行观测 spread 的 4.37 倍,但缺失 manifest 阻止完整配置平价与噪声地板声明。初次 E1 的 optimizer ownership 差异已修复，版本绑定 R12 的指定内存 trace 通过；但 E2 本轮未完成，R12 也没有验证 standalone checkpoint replay 或指标等价，因此不作更强归因,也不声称归约定理已被端到端经验演示。理论保证限于平稳 proposal 下的 `g=0` 精确归约及 proposal/单步转移行 TV 界,不延伸到端到端性能。
