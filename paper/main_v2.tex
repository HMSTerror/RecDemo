...
% One-line positioning: PreferGrow personalizes the score; we personalize
% the kernel --- safely.

The question has acquired urgency. A fast-moving line of work is making
discrete-diffusion corruption \emph{adaptive} for generative
recommendation: parallel semantic-ID decoding under masked diffusion
\cite{llada-rec}, confidence-ordered any-order decoding \cite{gr-diffgrm},
and position-adaptive masking schedules motivated by the observation
that uniform masking is harmful \cite{gr-admasked}, with analysis
beginning to follow \cite{gr-mdtheory} on top of the general
discrete-diffusion language-model program \cite{dllm-llada,sedd}. What
this wave adapts \emph{to} is largely model-internal signals
(confidence, position heuristics); what it lacks is a notion of
\emph{evidence pricing}---a train-computable statistic that says when
external evidence should be allowed to shape corruption at all---and
safety guarantees for the shaped process. This paper is a
pre-registered reliability study of exactly that question, in the
simplest kernel family where it can be posed cleanly.
...
the frozen PreferGrow run set. External baseline:
\pending{one of DiffuRec or RecFormer under the shared protocol
(CLOSE-04); if the compute window closes, report the absence
explicitly}~\cite{diffurec,recformer}.
...
$g\equiv 0$---the trained system is \emph{mathematically the host
...
\pending{v2 control arms under the frozen two-factor gate: Beauty
four arms completed on the clean root, Steam arms in flight (CLOSE-01);
fill from docs/reports/data/2026-07-06-sprint07/sprint07_control_table.csv
once archived locally, and scope claims to completed arms.}
...
\section{Proofs}
\label{app:proofs}
Full proofs of Props.~\ref{prop:validity}--\ref{prop:collision} and
Lemma~\ref{lem:tv} follow the frozen draft (rank-one kernel algebra:
$\Pi_a\Pi_b=\Pi_b$; matrix-exponential closed form of
Eq.~\eqref{eq:forward}; detailed balance w.r.t.\ the proposal; the
stationary-reversal ratio with $\pu$ substituted; the affine collision
identity). \pending{merge the complete appendix proofs from the frozen
draft paper/main.tex at CLOSE-07; statements are unchanged.}
...
$\pcore$ and never modified by the gate. \pending{merge full text from
the frozen draft at CLOSE-07.}
...
documentation, not benchmarks of the final system.
\pending{merge parameterization details from the frozen draft at
CLOSE-07.}
...
checklist is submitted separately.
\pending{finalize hardware/runtime details and supplement packaging at
CLOSE-07.}

\section{Negative results}
\label{app:results}
The metadata-sparsity robustness hypothesis was not supported and is
reported here as a negative result with its artifacts.
\pending{merge full text from the frozen draft at CLOSE-07.}
