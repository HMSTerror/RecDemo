...
%%     three-run spread because the run manifests are absent. E1 first
%%     stopped at step 0; the scoped repaired R12 trace later passed
%%     2,986 comparisons with zero failures.
...
separates the endpoint domains, while the two intermediate domains swap
adjacent ranks. This descriptive pattern motivates an exposure-risk
audit: text can define informative hard negatives, but when it predicts
future positives the same tilt creates false-negative pollution. We
operationalize this with a two-factor,
...
loss. The empirical evidence is deliberately tiered. A train-only
cross-domain discovery statistic motivates an observed next-positive
exposure proxy, which freezes a condition-level corruption-risk factor
before the atomic pilot. The historical four-domain runs are mixed
single-run observations, and their strict rank order does not match the
utility order because Beauty and ATG swap adjacent ranks. The terminal
atomic pilot is released only after its preregistered RISK-08 contract
is complete; no partial pilot metric is used here. Accordingly, our
contribution is a mechanism and reliability audit with kernel-scoped
guarantees, not a claim of uniform recommendation gains.
...
find an endpoint-consistent but not strictly rank-matched four-domain
pattern (Sec.~\ref{sec:inversion}): the most favorable outcome occurs
at the weakest next-item text signal and the least favorable at the
strongest, while the two intermediate domains swap adjacent ranks.

This discovery motivates a second, intervention-facing layer. We call
$\log(q_{\rm text}(y\mid h)+10^{-12})-
\log(q_{\rm core}(y)+10^{-12})$ the observed next-positive exposure
proxy (EPE). Controlled corruption then freezes a condition-level risk
factor $\phi_R$ before training. Thus $\Uds$ is a discovery statistic,
EPE is a mechanism measurement, and $\phi_R$ is the preregistered
intervention; they are not interchangeable. Together they motivate
\emph{fallback-safe evidence conditioning}. A low-capacity gate
\begin{equation*}
  g(\ut,R) \;=\; g_{\max}\;\phi_R\;\mathrm{clip}(\ut,0,1)
\end{equation*}
...
and a frozen condition-level risk factor $\phi_R$ derived under the
preregistered corruption contract. The gated kernel
...
Our contributions:
\begin{itemize}
  \item \textbf{The exposure-risk finding.} A train-only text utility
  statistic separates the two endpoint domains, while Beauty and ATG
  exchange adjacent ranks. We report this as a four-domain descriptive
  pattern rather than an exact rank law or confirmatory p-value. We give
  a mechanism-level account: corruption
...
\begin{figure}[t]
\centering
\input{figures/method_architecture.tex}
...
\begin{figure}[t]
\centering
\input{figures/risk_mechanism.tex}
...
\bottomrule
\end{tabular}
\caption{Train-only text utility $\Uds$ (popularity-negative AUC) and
the legacy first-generation outcome. The endpoint domains agree with
the exposure-risk interpretation, but Beauty and ATG swap adjacent
ranks; this is a four-domain descriptive pattern, not an exact rank law
or a significance test. The legacy hinge is distinct from the later
EPE/$\phi_R$ corruption intervention.}
...
Three readings. \emph{First}, the endpoints are consistent with the
exposure-risk account: Steam ($+0.0064$) has the lowest text utility,
whereas ML1M ($-0.0622$) has the highest. Strict ordering is not
satisfied because Beauty and ATG exchange adjacent ranks under both
the legacy and corrected evaluators. With four datasets this is
descriptive rather than a population law; no confirmatory inference is
attached to the earlier $1/24$ calculation. The statistic is train-computable,
...
the frozen PreferGrow run set. The external baseline is an adapted
common-contract SASRec implementation. It consumes the frozen
train/validation/test frames without a native resplit, uses full-softmax
training and full-real-catalog evaluation, and selects checkpoints by
validation NDCG@10. It is not a line-by-line or official-protocol
reproduction of SASRec. DiffuRec is excluded from the confirmatory
comparison; its historical artifacts remain archived.
...
\begin{tabular}{lrrrr}
\toprule
Dataset & Val HR@10 & Val NDCG@10 & Test HR@10 & Test NDCG@10 \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Adapted common-contract SASRec baseline, seed 100. All four
domains use the frozen row contract and validation-only checkpoint
selection. Beauty's validation-to-test decline is shown explicitly;
the single-run result is not interpreted as a general failure of
SASRec.}
\label{tab:sasrec}
\end{table}
...
The atomic pilot uses a separately preregistered condition-level risk
response: $\phi_R$ is $1.0/0.1366/0.0$ for Beauty and
$1.0/0.0581/0.0$ for Steam under c0/c60/c100. These values are frozen
interventions, not learned performance statistics.
...
The strict four-domain order is not satisfied because Beauty and ATG
swap adjacent ranks under both evaluators. The earlier $1/24$
exchangeability calculation is therefore not an empirical p-value for
the observed table. The within-dataset per-user follow-up does not
...
or fully manifested floor claim. E1 initially found a step-0
optimizer-ownership divergence. After the dated repair, the scoped R12
production trace at revision \texttt{0338cc2} completed 2,986
comparisons with zero failures. This does not certify every dataset,
checkpoint, or auxiliary injection path, and E2 remained unlaunched in
that sprint. We do not claim
