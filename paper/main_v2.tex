...
%%     three-run spread because the run manifests are absent. The initial
%%     E1 trace found a step-0 optimizer-ownership mismatch; after repairing
%%     that ownership, revision-scoped R12 passed 2,986 trace comparisons.
%%     Metric-equivalence and checkpoint-replay wording stays paused.
...
a legacy train-only next-item text-utility statistic gives a descriptive
inverse ordering of first-generation tilt outcomes. The ordering is
perfect on the archived measurement scale but becomes three-of-four
under the corrected common evaluator through an adjacent Beauty--ATG
swap; we therefore treat it as a discovery signal, not a population
law. We measure the proposed mechanism more directly with train-only
observed-next-positive exposure (EPE/PNE@10), and use a two-factor,
history-only gate $g=g_{\max}\sD\mathrm{clip}(\ut,0,1)$ to interpolate
between the host's learned proposal and a frozen text anchor. The frozen
dataset scale $\sD$ is generation-specific: the legacy four-domain hinge
and the dated controlled-corruption reliability scale are reported
separately. The design is \emph{fallback-safe} at the kernel level: it
reduces exactly to the host at $g=0$, while
$\TV(\pu,\pcore)\le g$ and one-step transition-row TV is at most
$(1-\alpha_\sigma)g$; these are not end-to-end performance bounds. After
repairing an optimizer-ownership mismatch, a revision-scoped E1 trace
passed 2,986 designated comparisons, without testing standalone
checkpoint replay or metric equivalence. In a seed-100
controlled-corruption precursor, Steam c60 moved validation and test
NDCG@10 in the same direction; Beauty's positive differences were
test-only, and c100 recovered the host best summary under an explicit
zero dataset scale. Missing anchor artifacts and user-level uncertainty
prevent an efficacy claim or a multi-run reliability assessment. The contribution is thus a
reliability audit and a kernel-scoped safe construction, not uniform
...
find a descriptive inverse ordering of first-generation text-tilting
outcomes on the archived four-benchmark measurement scale
(Sec.~\ref{sec:inversion}): the largest gain occurs on the dataset with
the \emph{weakest} next-item text signal, and the severe loss on the
dataset with the \emph{strongest}. The corrected common evaluator keeps
this endpoint contrast but swaps the adjacent Beauty and ATG positions,
so strict rank agreement is three-of-four rather than an unconditional
four-of-four replication.
...
\begin{equation*}
  g(\ut,D) \;=\; g_{\max}\;\sD\;\mathrm{clip}(\ut,0,1)
\end{equation*}
...
and a frozen dataset-level scale $\sD$. We keep three evidence objects
separate. Legacy $\Uds$ is a discovery descriptor and supplies the
original four-domain hinge $\phids$. RISK-03 EPE/PNE@10 measures how
much a frozen text proposal exposes the observed next positive and its
text neighborhood. The dated controlled-corruption pilot uses a
different pre-registered scale $\phi_R$, interpreted as evidence
retention under corruption. For the frozen Beauty/Steam values,
$\phi_R$ is increasing in EPE; it therefore does \emph{not} establish
the stronger claim that high positive exposure automatically closes the
gate. The gated kernel
...
rather than for one architecture. The legacy four-domain validation and
the controlled-corruption pilot are consequently reported as different
evidence generations. The former contains one closed-gate parity hit
(Beauty), one closed-gate miss (ML1M), one barely-open miss (ATG,
$\phids{=}0.117$), and a fully open directional gain below its reference
magnitude (Steam). The latter supplies seed-100 exposure and fallback
observations but is not RISK-08 eligible until its complete same-root
anchor matrix exists. Neither is treated as an end-to-end demonstration
of exact reduction or uniform gate efficacy.

Our contributions:
\begin{itemize}
  \item \textbf{A utility-inversion discovery and direct exposure
  audit.} A train-only text-utility statistic descriptively
  rank-orders legacy first-generation outcomes in the opposite
  direction on the archived four-domain scale; corrected-evaluator
  arbitration weakens strict agreement to three-of-four. Under
  exchangeability, $1/24$ is only the descriptive fraction for one
  specified four-label order, not a confirmatory p-value. We explain the
  false-negative channel and complement the rank descriptor with
  train-only EPE/PNE@10 observed-positive exposure measurements
  ...
  first, to our knowledge, discrete preference-diffusion recommender whose transition
  ...
  \item \textbf{A staged reliability methodology.} A length-matched
  random-history null calibrates user coherence; a train-only discovery
  statistic, a direct exposure audit, and a frozen controlled-corruption
  scale are then versioned rather than conflated. We preserve the
  implemented $\phi_R$ sign and narrow its interpretation instead of
  tuning it after outcomes are observed.
  \item \textbf{Bounded mechanism evidence.} First-generation controls
  and the controlled-corruption precursor expose where alignment,
  fallback, and evidence quality enter. Every seed-100 value is reported
  as a single-run observation; incomplete anchors and unestimable
  user-level uncertainty are retained as limitations rather than
  repaired with a favorable subset (Sec.~\ref{sec:mechanism}).
\end{itemize}
...
history-only gate $g=g_{\max}\,\sD\,\mathrm{clip}(\ut,0,1)$
...
\subsection{Dataset evidence: discovery, measurement, intervention}
\label{sec:uds}
The evidence chain has three versioned objects. First, for sampled train
transitions $(h,y)$, legacy $\Uds$ is the probability that anchor
similarity ranks the observed next item $y$ above a popularity-sampled
negative (an AUC; estimator frozen in Appendix~\ref{app:uds}). It is a
train-only discovery descriptor: no validation or test information
enters. The original four-domain instantiation maps it to
\begin{equation}
  \phi_U(\Uds) \;=\;
  \mathrm{clip}\!\big((0.70-\Uds)/0.10,\;0,\;1\big),
  \label{eq:phi}
\end{equation}
...
Second, RISK-03 measures the mass mechanism directly. For frozen text
and core proposals $q_{\mathrm{text}}(\cdot\mid h)$ and
$q_{\mathrm{core}}$, define observed next-positive exposure
\begin{equation}
  \mathrm{EPE}_{D,c} = \frac{1}{N}\sum_{(h,y)}
  \left[\log\!\big(q_{\mathrm{text},c}(y\mid h)+\epsilon\big)
  -\log\!\big(q_{\mathrm{core}}(y)+\epsilon\big)\right],
  \label{eq:epe}
\end{equation}
and PNE@10 as the mean text-proposal mass on the frozen ten-item text
neighborhood of $y$. EPE/PNE@10 are train-only exposure proxies. They
are neither a complete false-negative rate nor an end-to-end metric,
and the frozen records lack the user identifiers needed for user-level
bootstrap uncertainty.

Third, the Beauty/Steam controlled-corruption pilot freezes
\begin{equation}
  \phi_R(D,c)=\mathrm{clip}\!\left(
  \frac{R_{D,100}-R_{D,c}}{R_{D,100}-R_{D,0}},0,1\right),
  \qquad R_{D,c}=\mathrm{EPE}_{D,c}.
  \label{eq:phir}
\end{equation}
For both frozen ladders $R_{D,0}>R_{D,100}$, so the implemented mapping
is \emph{increasing} in EPE: clean evidence receives scale one and the
fully permuted bank scale zero. We therefore interpret $\phi_R$ as an
evidence-retention/corruption-reliability intervention, not as a
demonstrated rule that suppresses high EPE. We do not alter this sign or
its numbers after observing outcomes. In what follows $\sD$ denotes the
declared frozen scale---$\phi_U(\Uds)$ for the legacy four-domain study
or $\phi_R(D,c)$ for the dated controlled-corruption pilot---and the two
are never pooled as one result.
...
$g=g_{\max}\,\sD\,\mathrm{clip}(\ut,0,1)$,
\begin{equation}
\begin{aligned}
...
$\ut$, the declared frozen dataset scale $\sD$, and $\pu$, for the whole
trajectory.
...
\label{lem:tv}
The proposal and each row of the forward transition satisfy
\begin{align*}
  \TV(\pu,\pcore) &\le g, \\
  \TV\!\left(P_{\sigma,\pu}(i,\cdot),
  P_{\sigma,\pcore}(i,\cdot)\right) &\le (1-\alpha_\sigma)\,g.
\end{align*}
\end{lemma}
...
gate in the right-hand regime in the legacy four-domain instantiation.}
\label{fig:inversion}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Legacy-scale discovery table. Train-only text utility $\Uds$
(popularity-negative AUC) inversely rank-orders the archived test
outcome of first-generation user-gated kernel tilting, $4/4$ on that
measurement scale. Under the corrected common evaluator, an adjacent
Beauty--ATG swap weakens strict agreement to $3/4$ while preserving the
Steam/ML1M endpoint contrast. If the labels and all $4!$ orders are
treated as exchangeable, $1/24$ is only the descriptive fraction for
one specified order, not a confirmatory p-value.}
...
Three readings. \emph{First}, on the archived scale the ordering is
perfectly inverted: the only clear gain (Steam, $+0.0064$) occurs at the
\emph{lowest} text utility, and the severe loss (ML1M, $-0.0622$) at the
\emph{highest}. The corrected common evaluator preserves those endpoints
but swaps the adjacent Beauty/ATG positions, giving three-of-four strict
rank agreement. With four aggregate datasets this is a discovery rather
than a population law. Under exchangeability, one specified order among
$4!$ equally likely label orders gives the descriptive calculation
$1/24$; no confirmatory inference is attached. The statistic is
train-computable and motivates the exposure audit: a proposal is a
corruption law, so evidence that predicts positives can convert tilt
into observed-positive exposure, while evidence that merely structures
...
$\phi$-weighted) and \emph{none} of the observed harm. Each of the four
archived leave-one-domain-out subsets retains the legacy-scale ordering
($\rho=-1$); under
three-label exchangeability, each specified order has only the
descriptive fraction $1/6$.
...
NDCG@10 under the production checkpoint selector; model selection uses
validation only, while test metrics were logged during development and
are not described as an untouched final holdout. The host baseline is
the frozen PreferGrow run set. The confirmatory external reference is a
  minimal SASRec~\cite{sasrec} trained once per domain (seed 100) on the same mappings
and splits, with full-catalog evaluation and the same validation-only
selector. DiffuRec is excluded from the confirmatory comparison; its
archived wrapper results remain only as evaluator-audit provenance and
as related work~\cite{diffurec}. The dated E0 amendment establishes the
corrected common full-tail, row-weighted, real-item test contract but
does not recompute legacy validation selection. Table~\ref{tab:validation}
therefore remains a clearly labeled legacy-scale table in this cycle;
any later replacement must move all systems together rather than
selectively backfill one method.
...
dataset scale per the declared evidence generation) and infrastructure in
...
\begin{tabular}{lrrr}
\toprule
Dataset & best epoch & val NDCG@10 & test NDCG@10 \\
\midrule
...
\bottomrule
\end{tabular}
\caption{SASRec common-contract external reference. Each row is a
seed-100 single-run observation from one four-domain atomic group;
validation NDCG@10 selects the checkpoint and test is a development-time
readout. Beauty's large validation-to-test drop is unresolved and is
reported rather than used to claim superiority.}
% artifact: /data/Zijian/goal/aaai27_queue/2026-07-12-e05-sasrec-seed100-gpu0-c5c9280
% manifest sha256: 8ae6126352600dba1cf297fb7567ea7200e72023dd172785f0875fc831a2d8e0
\label{tab:sasrec}
\end{table}

SASRec is substantially stronger than the frozen host on Steam and
ML1M but weak on Beauty and ATG. This mixed external reference makes the
paper's scope explicit: we study reliability and exact kernel fallback
inside a deliberately simple diffusion host, not state-of-the-art
accuracy. One external architecture is still thin baseline coverage,
and no cross-model significance claim is made.
...
\bottomrule
\end{tabular}
\caption{Legacy frozen-gate validation, test NDCG@10 from the archived
p2-selected runs, one seed per the frozen protocol. The cells are kept
as a whole-table legacy scale rather than selectively mixed with E0 or
SASRec values. Each dataset is read against its pre-registered
prediction; no uniform-superiority claim is made or supported.}
...
claim that exact reduction has been demonstrated end-to-end. The initial
E1 trace found a step-0 optimizer-ownership divergence for the canonical
core proposal parameter. After aligning optimizer and EMA ownership, the
revision-scoped R12 trace passed 2,986 comparisons at steps 0, 1, 100,
and 1000 with no failure. R12 is implementation evidence for that
designated in-memory path, not a recommendation-metric equivalence or
standalone-checkpoint result. The downstream E2 attribution controls
were not completed in this sprint. The theoretical proposition is
...
%            docs/reports/data/2026-07-11-e01-gzero-production-trace-r12/e01_gzero_trace.json
%            docs/reports/data/2026-07-11-e01-r12-ownership-scope-amendment/e01_r12_ownership_scope_amendment.json
%            docs/reports/data/2026-07-10-close10-atg-noise-floor-provenance-limited/close10_atg_provenance_limited_report.json

\subsection{Observed-positive exposure and the controlled-corruption precursor}
\label{sec:controlled}

RISK-03 applies Eqs.~\eqref{eq:epe}--\eqref{eq:phir} to frozen train
transitions before any controlled-corruption training. Table~\ref{tab:epe}
reports the six selected point estimates. Positive EPE means that the
text proposal assigns more log mass to the observed next positive than
the frozen core proposal. PNE@10 provides a neighborhood-mass view of
the same channel; neither quantity is a recommendation metric.
...
\bottomrule
\end{tabular}
\caption{Train-only observed-next-positive exposure audit. Values are
frozen artifact point estimates; E7 could not estimate user-resampling
uncertainty because the selected transition records lack user IDs
(zero executed bootstrap replicates). The $\phi_R$ column is the frozen
evidence-retention intervention, not a fitted response to these results.}
% artifact: docs/reports/data/2026-07-12-r7-evidence-amendment/r6a_evidence_manifest.json
\label{tab:epe}
\end{table}

The precursor r6a queue completed the two hosts and six full arms before
the repaired anchor contract existed. Table~\ref{tab:r6a} places
validation and test together because validation selected each best
checkpoint and test metrics were logged during development.
...
\toprule
Dataset & arm & $\phi_R$ & val NDCG@10 & $\Delta$val & test NDCG@10 & $\Delta$test \\
\midrule
...
\bottomrule
\end{tabular}
\caption{r6a seed-100 single-run observations under the E0 full-tail
evaluator and validation-NDCG@10 p5 selector. Beauty c0/c60 are
test-only positive observations: their validation differences are
approximately zero. Steam c60 moves validation and test in the same
direction. c100 is an explicit $\phi_R=0$ production fallback sanity
check, not spontaneous collapse of the user factor.}
% artifact: docs/reports/data/2026-07-12-r7-evidence-amendment/EPE_PHI_R_METHOD_AMENDMENT.md
\label{tab:r6a}
\end{table*}

Under c100, each selected best-summary file is byte-identical to its
matched host, whereas the checkpoints differ because the full arm
serializes text-side state. This independently checks production
fallback under an explicit zero scale but is not a nontrivial efficacy
point. Steam c60 is the strongest completed directional observation.
Beauty's c0/c60 differences are especially weak evidence in light of
their validation parity and development-time test logging. Moreover,
r6a has no per-run artifact manifests, its run-local logs are empty, and
all six anchor tasks failed closed before training; it cannot satisfy
the original same-root RISK-08 contract. A repaired fourteen-task r7
attempt is therefore required. No r7 metric or exit is included here
before its immutable RISK-08 artifact exists.
...
readouts do not validate a final-v2 corruption-response claim. No valid
preregistered E3 full-arm result was available in this cycle; a dated
queue attempt stopped before any full arm after asset/protocol checks failed.
...
an empirical discovery we believe is of independent interest: because
kernel proposals are corruption laws, text evidence that predicts
positives can be hazardous when used as corruption mass. Legacy $\Uds$
provides a train-only rank descriptor, while EPE/PNE@10 audits observed
next-positive exposure directly. The frozen dataset scale is versioned
with its evidence generation rather than treated as a universally
validated risk gate.

\emph{Limitations.} The cross-dataset inversion rests on four aggregate
points and is perfect only on the archived measurement scale; the
corrected common evaluator swaps adjacent Beauty/ATG and reduces strict
rank agreement to three-of-four while preserving the endpoint contrast.
...
or fully manifested floor claim. The initial E1 optimizer-ownership
divergence was repaired, and revision-scoped R12 then passed its
designated in-memory trace; E2 was nevertheless not completed in this
sprint, and R12 did not test standalone checkpoint replay or metric
equivalence. We do not claim an end-to-end empirical demonstration of exact reduction or a stronger
causal attribution. The controlled-corruption $\phi_R$ values are
increasing in observed EPE and are therefore evidence-retention scales,
not evidence that high EPE automatically closes the gate. r6a provides
one-seed precursor observations only: Steam c60 moves validation and test
together, Beauty's positive differences are test-only, and explicit-zero
c100 is a fallback sanity check. Its missing anchor artifacts, empty
run-local logs, and absent RISK-08 exit prevent an efficacy claim; E7
also executed zero bootstrap replicates because user IDs were
unavailable. The common-contract SASRec baseline is limited to one
architecture and has an unresolved Beauty validation-to-test drop. Test
metrics were logged during development, so none of these test readouts
is an untouched final holdout. The mechanism evidence comes from the
...
\section{Proofs}
\label{app:proofs}
Throughout, fix the user history $h$, the gate inputs $\ut$ and the
declared frozen scale $\sD$, hence $g$ and the proposal $\pu$ of
Eq.~\eqref{eq:pu}; $\pu$ is
...
\label{app:impl}
Hyperparameters: $\tau_0{=}0.2$, $g_{\max}{=}0.5$, $k{=}2.0$, frozen
$\phi_U(0.70,0.10)$ for the legacy study; controlled $\phi_R$ values
Beauty $1/0.136631/0$ and Steam $1/0.058081/0$; selector: validation
NDCG@10 (p5); banks:
sentence-t5-xl, frozen and hash-recorded; splits regenerated from raw
data under a recorded protocol. Dated contract artifacts record config,
seed, bank/null-curve/split/evidence hashes. The r6a precursor is an
explicit exception: its selected summaries are hash-bound, but its
per-run artifact manifests and nonempty run-local logs are absent, so it
cannot enter RISK-08. E7 is likewise incomplete: the requested bootstrap
count was 1000, the executed count was zero, and uncertainty is
\texttt{not\_estimable} because selected transitions lack user IDs.
