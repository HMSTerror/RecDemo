...
DEFAULT_RUN_ROOT = Path("/data/Zijian/goal/RecDemoRuns/close10_atg_noise_floor")
DEFAULT_OFFICIAL_REPO_ROOT = Path("/data/Zijian/goal/RecDemo_clean_closeout_chain")
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "reports" / "data" / f"{date.today().isoformat()}-close10-atg-noise-floor"
...
def run_dir_for(run_root: Path, seed: int) -> Path:
    return run_root / f"atg_core_seed{seed}"


def summary_path_for(run_root: Path, seed: int) -> Path:
    return run_dir_for(run_root, seed) / "checkpoints-meta" / "ATG" / "best_summary_hybrid.json"


def manifest_path_for(run_root: Path, seed: int) -> Path:
    return run_dir_for(run_root, seed) / "checkpoints-meta" / "ATG" / "frozen_run_manifest.json"


def log_path_for(run_root: Path, seed: int) -> Path:
    return run_dir_for(run_root, seed) / "logs" / f"atg_core_seed{seed}.log"
...
def read_gate1_atg_delta(gate1_report_path: Path) -> dict[str, object]:
    payload = load_json(gate1_report_path)
    if payload is None:
        return {}
    for row in payload.get("datasets", []):
        if str(row.get("dataset")) == "ATG":
...
def build_decision_line(noise_floor_summary: dict[str, object], gate1_context: dict[str, object]) -> dict[str, object]:
    ...
    if float(max_pairwise) >= float(gate1_abs):
        return {
            ...
            "reason": "measured_host_noise_floor_covers_gate1_atg_delta",
        }
    return {
        "decision_line": "outside_noise_red_flag",
        "reason": "gate1_atg_delta_exceeds_measured_host_noise_floor",
    }
...
def build_markdown(
    ...
    lines = [
        ...
        f"- 远端执行根要求: `{fmt_path(official_repo_root)}`",
        f"- 最大成对 |delta test p2 NDCG@10|: {fmt(max_p2) if max_p2 is not None else '待更多 completed seeds'}",
    ]
    if gate1_context:
        lines.append(f"- Gate-1 ATG 官方 delta_test_p2_ndcg10: {fmt(float(gate1_context['delta_test_p2_ndcg10']))}")
...
def write_markdown(
    ...
    path.write_text(
        build_markdown(rows, pairwise, noise_floor_summary, gate1_context, decision_line, official_repo_root),
        encoding="utf-8",
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the CLOSE-10 ATG host-noise-floor dated report.")
...
def main() -> None:
    ...
    gate1_context = read_gate1_atg_delta(args.gate1_report_path)
    decision_line = build_decision_line(noise_floor_summary, gate1_context)

    args.output_dir.mkdir(parents=True, exist_ok=True)
    output_csv = args.output_dir / "close10_atg_noise_floor_table.csv"
    output_json = args.output_dir / "close10_atg_noise_floor_report.json"
    output_md = args.output_dir / "close10_atg_noise_floor_report_zh.md"
