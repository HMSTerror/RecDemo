...
write_run_manifest() {
  ...
  RUN_DIR="$run_dir" \
  RANDOM_SEED="100" \
  TEXT_KERNEL_VERSION="$TEXT_KERNEL_VERSION" \
  TEXT_TEMPERATURE="$TEXT_TEMPERATURE" \
  TEXT_G_MAX="$TEXT_G_MAX" \
  TEXT_AGREEMENT_K="$TEXT_AGREEMENT_K" \
  TEXT_AGREEMENT_WEIGHT="$TEXT_AGREEMENT_WEIGHT" \
  TEXT_COMPLETENESS_WEIGHT="$TEXT_COMPLETENESS_WEIGHT" \
  TEXT_HISTORY_RELIABILITY_WEIGHT="$TEXT_HISTORY_RELIABILITY_WEIGHT" \
  TEXT_ESS_WEIGHT="$TEXT_ESS_WEIGHT" \
  TEXT_RECENCY_WEIGHT="$TEXT_RECENCY_WEIGHT" \
  TEXT_STABILITY_WEIGHT="$TEXT_STABILITY_WEIGHT" \
  TEXT_MAX_TEMPERATURE_SCALE="$TEXT_MAX_TEMPERATURE_SCALE" \
  TEXT_MIN_PSEUDO_MASS="$TEXT_MIN_PSEUDO_MASS" \
  TEXT_POPULARITY_MIX_SCALE="$TEXT_POPULARITY_MIX_SCALE" \
  TEXT_POPULARITY_MIX_POWER="$TEXT_POPULARITY_MIX_POWER" \
  TEXT_PSEUDO_MASS_SCALE="$TEXT_PSEUDO_MASS_SCALE" \
  TEXT_PSEUDO_MASS_POWER="$TEXT_PSEUDO_MASS_POWER" \
  TEXT_CENTER_EMBEDDINGS="$TEXT_CENTER_EMBEDDINGS" \
  TEXT_ABLATION_MODE="$TEXT_ABLATION_MODE" \
  TEXT_INJECTION_MODE="$TEXT_INJECTION_MODE" \
  WRITE_SNAPSHOT_CHECKPOINT="$WRITE_SNAPSHOT_CHECKPOINT" \
  WRITE_BEST_CHECKPOINT="$WRITE_BEST_CHECKPOINT" \
  EARLY_STOP_METRIC="$EARLY_STOP_METRIC" \
  EARLY_STOP_STRENGTH="$EARLY_STOP_STRENGTH" \
  "$PYTHON_BIN" - <<'PY'
import hashlib
import json
import os
