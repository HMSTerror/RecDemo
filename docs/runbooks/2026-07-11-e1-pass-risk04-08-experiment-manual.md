# E1 通过后 RISK-04～08 seed-100 实验手册

_PreferGrow AAAI-27；日期：2026-07-11（Asia/Shanghai）；本手册只定义可审计的资产与队列边界，不自动启动训练。_

---

## 🧭 当前状态与授权边界

E1 已在 R12 dated attempt 中通过：`random_seed=100`，trace steps 为 `0,1,100,1000`，FP32 tolerance 为 `1e-6`，`comparisons=2986`，`failed_comparisons=0`，`first_divergence=null`。证据位于 [R12 trace report](../reports/data/2026-07-11-e01-gzero-production-trace-r12/e01_gzero_trace.json)、[RISK-02 pass marker](../reports/data/2026-07-11-e01-gzero-production-trace-r12/RISK-02_PASS.json) 和 [R12 attempt manifest](../reports/data/2026-07-11-e01-gzero-production-trace-r12/attempt_manifest.md)。这些证据只解除 E1 implementation gate；它们不等价于 RISK-04、RISK-05、RISK-08 或 method-pass 通过。
...
flowchart TB
    ...
    e1([E1 pass marker]) --> banks[Build 12 dated banks]
    ...
    manifest --> validate[Controller validate and dry-run]
    validate --> pilot[Later authorized seed-100 pilot]
    ...
    class banks,freeze,manifest,validate,pilot,decision action
    class severe gate
    class stop stop
```
...
RISK-05 只能绑定已经验证的 RISK-04 bundle、train-only preflight 报告和一个终态 E1 marker。preflight 必须携带 Beauty/Steam 六个 level 的 bank hashes；builder 会拒绝缺失或不匹配的 bank hash。冻结协议包含 EPE/PNE@10、`phi_R`、相邻 reversal、Spearman 和 worst-delta 的阈值，且不写入 validation/test metric；阈值一旦写入 dated root，不得根据 pilot 结果回改。
...
先准备只包含 queue 运行时位置、source revision、ledger hash、evaluator/selector 配置和 Beauty/Steam 数据位置的 protocol JSON，再执行：
...
python -m scripts.aaai27_queue.cli validate `
  --queue-root 'E:\PreferGrow\dated\queue-2026-07-11\runs' `
...
在 Linux/l20 上，`--queue-root` 必须替换为 manifest 中的绝对 POSIX `run_root`；不能把 Windows 路径或工作站路径混入远端 manifest。`validate` 和 `dry-run` 是只读检查。只有用户另行明确授权且 RISK-04/05 marker、资源预检和 controller gate 全部通过后，才允许进入 pilot launch 边界。

## 🧾 训练授权、监控与产物要求

本轮实现不会自动执行下面的 `run` 命令。未来授权后，controller 只能消费已验证的 `queue/queue_seed100.json`，并在每张 L20 卡上同时保持最多一个训练进程。GPU0 的 CLOSE-10 仍然独立运行，不得杀进程、抢卡或删除目录。
...
QUEUE_RUN_ROOT=/data/Zijian/goal/aaai27_queue/2026-07-11/runs
...
本手册没有把计划数字当作结果；如果某个产物不存在，论文表格应写 `NA` 和原因，不能推断或补造数值。
