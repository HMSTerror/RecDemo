...
### 8.5 修订 D(2026-07-04):恢复连接后的执行顺序与外部 baseline 缺口

依据 accept 差距评估(决定性变量排序:FOLLOWUP-09 ≫ ASO > 外部 baseline ≥ 种子),恢复 l20 连接后按以下顺序执行(0/1/2 可同日并行,GPU 与离线分析不冲突):

0. **U_ds(ASO)**(§8.4,纯离线几分钟)→ 立即得出 ASO 开跑决定;
1. **FOLLOWUP-09 用户级效用-伤害分析**(离线,无训练)— 唯一能把反转发现从 n=4 暗示升级为用户级证据的杠杆,优先级高于一切训练;
2. **SPRINT-05 四数据集冻结门控验证跑**(+ ASO 若 §8.4 判开)— GPU 并行,≤1 天;
3. **外部 baseline(新增缺口)**:在四个 regenerated 数据集上以共享 split/selector 跑至少一个外部方法(DiffuRec 或 RecFormer,按与现有管线的集成成本择一)— 当前论文只有 host-relative 对照,这是 accept 线上方最重的一块石头;
4. **SPRINT-11 三种子**(时间允许);
5. **SPRINT-12 Gate 2 定档**(2026-07-14 不动)。

对应账本行:FOLLOWUP-09 升为 P0;新增 FOLLOWUP-11(ASO step-0)、FOLLOWUP-12(条件 ASO 验证跑)、FOLLOWUP-13(外部 baseline)。
